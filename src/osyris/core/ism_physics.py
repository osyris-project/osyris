...
def read_opacity_table(fname):
    ...
    theTable["nx"] = np.array(
        utils.read_binary_data(fmt="3i", content=data, increment=False))
    ...
    offsets["n"] += 9
    offsets["d"] += 1
    theTable["dens"] = utils.read_binary_data(fmt="%id" % theTable["nx"][0],
                                              content=data,
                                              offsets=offsets,
                                              increment=False)
    offsets["n"] -= 1
    ...
    offsets["d"] += 1
    theTable["tgas"] = utils.read_binary_data(fmt="%id" % theTable["nx"][1],
                                              content=data,
                                              offsets=offsets,
                                              increment=False)
    offsets["n"] -= 1
    ...
    offsets["d"] += 1
    theTable["trad"] = utils.read_binary_data(fmt="%id" % theTable["nx"][2],
                                              content=data,
                                              offsets=offsets,
                                              increment=False)
    offsets["n"] -= 1
    ...
    offsets["d"] += 1
    theTable["kappa_p"] = np.reshape(utils.read_binary_data(fmt=array_fmt,
                                                            content=data,
                                                            offsets=offsets,
                                                            increment=False),
                                     theTable["nx"],
    ...
    offsets["d"] += 1
    theTable["kappa_r"] = np.reshape(utils.read_binary_data(fmt=array_fmt,
                                                            content=data,
                                                            offsets=offsets,
                                                            increment=False),
                                     theTable["nx"],
...
def get_opacities(dataset, fname, variables={"kappa_p": "cm^2/g", "kappa_r": "cm^2/g"}):
    ...
    if "radiative_temperature" not in dataset["hydro"]:
        ...
        dataset["hydro"]["radiative_temperature"] = values = (
            dataset["hydro"]["radiative_energy_1"] / units["radiation_constant"])**.25
        print(" done!")
    pts = np.array([
        np.log10(dataset["hydro"]["density"].values),
        np.log10(dataset["hydro"]["temperature"].values),
        np.log10(dataset["hydro"]["radiative_temperature"].values)
    ]).T
    for var in variables:
        print("Interpolating " + var + "...", end="")
        vals = ism_interpolate(dataset.meta["opacity_table"],
                               dataset.meta["opacity_table"][var], pts)
        print(" done!")
...
def read_eos_table(fname):
    ...
    data_fields = [
        "rho_eos", "ener_eos", "temp_eos", "pres_eos", "s_eos", "cs_eos", "xH_eos",
        "xH2_eos", "xHe_eos", "xHep_eos"
    ]
    ...
    # Get table dimensions
    theTable["nx"] = np.array(
        utils.read_binary_data(fmt="2i", content=data, increment=False))

    # Get table limits
    offsets["i"] += 2
    offsets["d"] += 1
    [
        theTable["rhomin"], theTable["rhomax"], theTable["emin"], theTable["emax"],
        theTable["yHe"]
    ...
    for i in range(len(data_fields)):
        theTable[data_fields[i]] = np.reshape(utils.read_binary_data(fmt=array_fmt,
                                                                     content=data,
                                                                     offsets=offsets,
                                                                     increment=False),
                                              theTable["nx"],
...
def get_eos(
    ...
    pts = np.array([
        np.log10(dataset["hydro"]["density"].values),
        np.log10(dataset["hydro"]["internal_energy"].values /
                 dataset["hydro"]["density"].values)
    ]).T
    for var in variables:
        print("Interpolating " + var + "...", end="")
        vals = ism_interpolate(dataset.meta["eos_table"],
