program amr2cell
  ...
  real(KIND=8)::scale_l,scale_d,scale_t,xcenter,ycenter,zcenter
  ...
  real(KIND=8) :: rhomin,rhomax,Emax,emin,yHe,Tmax,Tmin,eint,tp_eos,ww,xx,yy,zz,kb=1.380658d-16
           ...
           do ind=1,twotondim
              ...
              !        & (x(i,3)-dx2)<=zmax
                 ...
                 if(ok_cell)then
                    ...
                    call temperature_eos(var(i,ind,1)*ud,eint,tempi,ht)
...
subroutine process_data(switch)
                 ...
                 if(ok_cell)then
                    ...
                    if (switch==1) then
                       ...
                       if(var(i,ind,1) > maxrho)then
                          ...
                          varmax(5) = dx
                          varmax(6) = real(icpu)
                          varmax(7) = real(ilevel)
                          do ivar=1,nvarh
                             varmax(7+ivar) = var(i,ind,ivar)
                          enddo
                          varmax(nvarh+8) = var(i,ind,11)*ud*((ul/ut)**2)*ud/(var(i,ind,1)*ud*kb)
                          if (eos.eq.1)then
                             eint = var(i,ind,11)*ud*((ul/ut)**2)/(gamma-1.0d0)
                             call temperature_eos(var(i,ind,1)*ud,eint,tp_eos,ht)
                             varmax(nvarh+8)=tp_eos
                          end if
                       endif
                    endif
                   ...
                   if (switch==2) then
                      rhoi = var(i,ind,1)*ud 
                      if(rhoi > rhomin2)then
                         mcore2 = mcore2 + rhoi*(dx*ul*boxlen)**3
                         acore2 = acore2 +  (rhoi*(dx*ul*boxlen)**3)*&
                              &        ((x(i,1)-varmax(2))*ul*boxlen*var(i,ind,3)*ul/ut&
                              &        -(x(i,2)-varmax(3))*ul*boxlen*var(i,ind,2)*ul/ut)
                      endif
                      if(rhoi > rhomin1)then
                         mcore1 = mcore1 + rhoi*(dx*ul*boxlen)**3
                         acore1 = acore1 +  (rhoi*(dx*ul*boxlen)**3)*&
                              &        ((x(i,1)-varmax(2))*ul*boxlen*var(i,ind,3)*ul/ut&
                              &        -(x(i,2)-varmax(3))*ul*boxlen*var(i,ind,2)*ul/ut)
                      endif
                   endif
...
!                     write(20,'(80(es16.6e3,1x))') x(i,1),x(i,2),x(i,3),dx,real(icpu),real(ilevel),&
!                          & (var(i,ind,ivar),ivar=1,nvarh)
